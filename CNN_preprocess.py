...
model.add(Flatten())
model.add(Dense(input_dim=64*7*7, output_dim=128))
# model.add(BatchNormalization())
model.add(Activation('relu'))
...
#    print('X_train[{1}] => \n{0}').format(X_train[display].astype('uint8'),display)
#    print('X_batch[{1}] => \n{0}').format(X_batch[display].astype('uint8'),display)
#    exit()

model.load_weights('/Users/will/Desktop/eric/ep7/predicted_cnn.csv.024-0.1167-0.9679.hdf5')
print("Training...")
if (VALIDATION_SIZE):
    ...
    model.fit_generator(datagen.flow(X_train, y_train,
                        ...
                        verbose=1,callbacks=[checkpointer, stopping])
else:
